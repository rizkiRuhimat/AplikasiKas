...
<body>
    ...
    LEFT JOIN `data_nasabah` AS b ON a.`id_nasabah`=b.`id_nsbh`
    LEFT JOIN `jenis_trx` AS c ON a.`tipe`=c.id_trx
    WHERE 1=1 and a.cat = 'out' AND MONTH(`date`) = DATE_FORMAT(NOW(),'%M %Y')";
    $result = $conn->query($sql);
    while ($row = $result->fetch_assoc()) {
        $total2 = $row['total'];
        $info2 = $row['info'];
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col">
                    ...
                    <?php
                    $get    =   "SELECT 
                    a.`no_id`,a.`name`,a.`kas`,b.periode,
                    CASE TRIM(flag) WHEN 0 THEN 'Cash' WHEN 1 THEN 'Transfer' END AS jenis_trx ,
                    5000*COUNT(b.id) AS subTotal 
                    FROM `imported_report` AS a
                    LEFT JOIN `imported_trx` AS b ON a.`no_id`=b.`no_id`
                    WHERE 1=1
                    AND a.kas = 'Ya'
                    AND periode = month(now())
                    GROUP BY a.id
                    ORDER BY a.id ASC ";
